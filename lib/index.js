...
class RankCard {
	 ...
	 */
	_calculateProgress() {
		const cx = this._data.progress.currentXP;
		const rx = this._data.progress.requiredXP;

		if (rx <= 0) return 1;
		if (cx > rx) return this._data.progress.width;

		let width = (cx * 615) / rx;
		if (width > this._data.progress.width) width = this._data.progress.width;
	...
	async build(fonts = { bold: 'Neutra', regular: 'Neutra' }) {
		...
		if (this._data.tag.display) {
			...
			ctx.font = `${this._data.tag.fontSize}px ${fonts.bold}`;
			ctx.fillStyle = this._data.tag.fontColour;

			ctx.fillText(tag, this._data.progress.x + 10, this._data.progress.y - 20);
		}
		...
		if (this._data.progress.display) {
			...
			// Show progress
			ctx.font = `bold ${this._data.progress.fontSize}px ${fonts.bold}`;
			ctx.fillStyle = this._data.progress.fontColour;

			ctx.fillText(progressText, this._data.progress.x + this._data.progress.width - ctx.measureText(progressText).width, this._data.progress.y - 20);
		}
		...
		if (this._data.level.display) {
			ctx.font = `${this._data.level.fontSize}px ${fonts.bold}`;
			ctx.fillStyle = this._data.level.fontColour;
			ctx.fillText(`Level ${abbreviate(this._data.level.value)}`, this._data.progress.x + 10, this._data.progress.y - 100);
		}

		// Draw the rank
		if (this._data.rank.display) {
			ctx.font = `bold ${this._data.rank.fontSize}px ${fonts.bold}`;
			ctx.fillStyle = this._data.rank.colour;
			ctx.fillText(
				`Rank ${abbreviate(this._data.rank.value)}`,
				this._data.progress.x + this._data.progress.width - ctx.measureText(progressText).width,
				this._data.progress.y - 100
			);
		}
